...
public class PlaybackOverlayFragment extends android.support.v17.leanback.app.PlaybackOverlayFragment {
    ...
    private int getDuration() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
            mmr.setDataSource(HomeActivity.titleFds.get(movie.getTitle()).getFileDescriptor(), HomeActivity.titleFds.get(movie.getTitle()).getStartOffset(),
                    HomeActivity.titleFds.get(movie.getTitle()).getLength());
        } else {
                mmr.setDataSource(HomeActivity.titleFds.get(movie.getTitle()).getFileDescriptor());
        }
